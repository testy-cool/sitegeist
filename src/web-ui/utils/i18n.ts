...
// Vendored into sitegeist: the extension augments i18nMessages with its own keys and calls
// setTranslations again with the full merge (see src/utils/i18n-extension.ts). This base layer
// is deliberately a subset, so it cannot satisfy the augmented interface on its own.
setTranslations(translations as unknown as Record<string, i18nMessages>);
