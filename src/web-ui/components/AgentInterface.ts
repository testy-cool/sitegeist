...
export class AgentInterface extends LitElement {
	...
	// The agent appends completed messages by mutating state.messages in place, so the array
	// reference stays the same across a turn (proven: length grows, reference does not change).
	// Binding that live array straight into <message-list> means Lit's reference-identity change
	// detection never fires, and the finished message silently fails to appear once the streaming
	// container is cleared on message_end. Snapshot instead, and only when the contents actually
	// change, so the list is still skipped for every token during streaming.
	private _messagesSource?: AgentMessage[];
	private _messagesSnapshot: AgentMessage[] = [];

	private get stableMessages(): AgentMessage[] {
		const live = this.session?.state.messages ?? [];
		if (live !== this._messagesSource || live.length !== this._messagesSnapshot.length) {
			this._messagesSource = live;
			this._messagesSnapshot = live.slice();
		}
		return this._messagesSnapshot;
	}
	...
	private setupSessionSubscription() {
		...
		// streamFn is constructor-only as of pi-agent-core 0.84 and can no longer be swapped in
		// after the fact. Sitegeist always passes its own proxy-aware streamFn when it constructs
		// the Agent (see createAgent in src/sidepanel.ts), so there is nothing to default here.
		...
		this._unsubscribeSession = this.session.subscribe(async (ev: AgentEvent) => {
			switch (ev.type) {
				...
				case "agent_end":
					...
					this.requestUpdate();
					// state.isStreaming is still true here. As of pi-agent-core 0.84 the run is only
					// idle once every agent_end listener has settled, and the flag is cleared after
					// that, in finishRun(), with no further event emitted. Since these listeners are
					// the only thing that re-renders, the editor would keep painting the Stop button
					// forever. Note this must not be awaited: finishRun() runs after the listeners
					// resolve, so awaiting idle from inside one deadlocks the run.
					void this.session?.waitForIdle().then(() => this.requestUpdate());
					break;
				case "message_update":
					if (this._streamingContainer) {
